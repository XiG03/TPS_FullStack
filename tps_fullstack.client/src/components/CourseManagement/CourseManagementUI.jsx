...
const CourseManagementUI = ({ courses, isLoading, error, onDeleteCourse }) => {
    ...
    const filteredCourses = courses.filter((course) => {
        ...
        return (
            course.ten?.toLowerCase().includes(normalizedSearch) ||
            course.mota?.toLowerCase().includes(normalizedSearch)
        );
    });
            ...
            <header className="flex flex-col gap-6 px-6 pt-8 pb-8 lg:flex-row lg:items-end lg:justify-between lg:px-12 lg:pt-10">
                <div className="flex flex-col gap-2">
                    ...
                    <p className="font-body text-on-surface-variant text-base">
                        Tổng quan khóa học, giảng viên và chương trình đào tạo.
                    </p>
                </div>
                <button
                    ...
                    {isLoading ? (
                        ...
                        filteredCourses.map((course) => (
                            ...
                            >
                                ...
                                <div className="font-body text-sm text-on-surface-variant line-clamp-2" title={course.mota}>
                                    {course.mota || 'Không có mô tả'}
                                </div>
                                <div className="flex items-center justify-end gap-1">
                                    <button
                                        onClick={() => navigate(`/courses/${course.khoahocID}`)}
                                    ...
                                    <button
                                        onClick={() => navigate(`/courses/${course.khoahocID}/edit`)}
                                        className="text-on-surface-variant hover:text-primary hover:bg-surface-container-highest p-2 rounded-md transition-colors"
                                        title="Chỉnh sửa"
                                    >
                                    ...
                                    <button
                                        onClick={() => onDeleteCourse(course.khoahocID)}
