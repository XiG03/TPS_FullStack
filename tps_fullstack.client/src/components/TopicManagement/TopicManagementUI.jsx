        ...
        <div className="flex-1 overflow-y-auto bg-surface relative z-0 flex flex-col min-h-[calc(100vh-60px)]">
            <header className="flex justify-between items-end px-12 pt-10 pb-10">
                <div className="flex flex-col gap-2">
                    <h1 className="font-headline text-4xl font-extrabold text-on-surface tracking-tight">Quản lý Chuyên đề</h1>
                    <p className="font-body text-on-surface-variant text-base">Tạo, chỉnh sửa và quản lý các chuyên đề học tập.</p>
                </div>
                <button 
            ...
            <section className="px-12 pb-16 flex-1 flex flex-col">
                <div className="grid grid-cols-[3fr_4fr_auto] gap-4 px-6 pb-4 font-label text-xs font-bold text-on-surface-variant uppercase tracking-wider">
                ...
                <div className="flex flex-col gap-3">
                    {isLoading ? (
                        ...
                        <div className="text-center py-10 font-body text-on-surface-variant">Không tìm thấy chuyên đề nào.</div>
                    ) : (
                        filteredTopics.map((topic) => (
                            <div key={topic.maID} className="bg-surface-container-lowest rounded-xl p-5 flex items-center shadow-[0_4px_12px_rgba(25,28,30,0.02)] transition-transform hover:-translate-y-0.5 hover:shadow-[0_8px_24px_rgba(25,28,30,0.04)] grid grid-cols-[3fr_4fr_auto] gap-4">
                                <div className="font-headline font-bold text-on-surface text-lg truncate">
                                    {topic.ten}
                                </div>
                                <div className="font-body text-sm text-on-surface-variant truncate" title={topic.mota}>
                                    {topic.mota || 'Không có mô tả'}
                                </div>
                                <div className="flex items-center justify-end gap-1 w-[120px]">
                                    <button onClick={() => navigate(`/topics/${topic.maID}`)} className="text-on-surface-variant hover:text-primary hover:bg-surface-container-highest p-2 rounded-md transition-colors" title="View Detail">
                                        <span className="material-symbols-outlined text-xl">visibility</span>
                                    </button>
                                    <button onClick={() => navigate(`/topics/${topic.maID}/edit`)} className="text-on-surface-variant hover:text-primary hover:bg-surface-container-highest p-2 rounded-md transition-colors" title="Edit">
                                        <span className="material-symbols-outlined text-xl">edit</span>
                                    </button>
                                    <button onClick={() => onDeleteTopic(topic.maID)} className="text-on-surface-variant hover:text-error hover:bg-error-container p-2 rounded-md transition-colors" title="Delete">
